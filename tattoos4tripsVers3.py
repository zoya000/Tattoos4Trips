...
tattoo7 = [7, "any sort of snowflake (intricate or minimalistic)"]
...
tattoo24 = [24, "solar system or arrangement of planets"]

# determines the adventurous scale of the vacation. would you like to go somewhere extreme? 'chillax'?
adventureTattoo = {"adventurous": [tattoo1, tattoo2], "exploring": [tattoo3, tattoo4], "lazy": [tattoo5, tattoo6]}
...
def hobbyChecker():
    hobbyCheck = input(
        "Do you have any hobbies you enjoy doing that you would want factored into the results of the"
        " quiz? \n \n(yes or no)\n")

    if hobbyCheck == "yes" or hobbyCheck == "YES":
        hobbies = input(
            "Please choose up to three of the following hobbies that you actively enjoy for the most accurate "
            "results.\n(To save your choice(s), type the associated number key for each hobby and separate them "
            "with spaces!)\n" + str(
...
def countTattoo(Tattoo, TattooCount):
    ...
    while not three:
        ...
        topTatsRaw = input("\nOut of the ones described above, which three tattoo designs would you want the most?\n("
                           "answer using the number key assigned to the description, please separate using "
                           "spaces)\n") + " "
    ...
    for Tat in topTats:
        for key, values in Tattoo.items():
            for value in values:
                if int(Tat) == int(value[0]):
                    TattooCount[key] += 1
    return TattooCount
